...
const BusInfo = ({route}) => {
  ...
  useEffect(() => {
    console.log(route.params.bus);
    fetch(`http://192.168.29.89:3000/add/spec/${route.params.bus}`)
      .then(res => {
        return res.json();
      })
      .then(data => {
  ...
  const findSeats = () => {
    ...
    fetch('http://192.168.29.89:5000/process_video')
      ...
      .then(data => {
        ...
        seatsAvailable(seats - totalPeopleInside);
        // Update seats available in MongoDB via another API request
        fetch(`http://192.168.29.89:3000/add/update/${route.params.bus}`, {
